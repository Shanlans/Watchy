...
void Watchy::handleButtonPress() {
  ...
  // Initialize button pins based on hardware version
...
void Watchy::showMenu(byte menuIndex, bool partialRefresh) {
  display.epd2.asyncPowerOn(); // 确保显示面板通电
  ...
  display.display(partialRefresh); // 触发实际显示
...
void Watchy::showHeartrate() {
  display.init(0, false, 2000, false); // Initialize the display
  display.fillScreen(GxEPD_BLACK);
  display.setTextColor(GxEPD_WHITE);
  ...
  // Set GUI state to APP_STATE to prevent unexpected deep sleep
  guiState = APP_STATE;
  
  MAX30105 sensor; // Create sensor object (using MAX30105 as it's more common)
  ...
  if (!sensor.begin()) {
    ...
    Serial.println("DEBUG: SUCCESS - MAX30105 sensor initialized");
  }

  // Configure the sensor with recommended settings
  const auto kSamplingRate = sensor.SAMPLING_RATE_400SPS; // 400 samples per second
  const float kSamplingFrequency = 400.0;
  
  sensor.setSamplingRate(kSamplingRate); // Set the sampling rate
  ...
  display.print("(Press any btn to cancel)");
  display.display(false); // full refresh

  // Constants for advanced algorithm
  const unsigned long kFingerThreshold = 10000;
  const unsigned int kFingerCooldownMs = 500;
  const float kEdgeThreshold = -2000.0;
  const float kLowPassCutoff = 5.0;
  const float kHighPassCutoff = 0.5;
  const bool kEnableAveraging = false; // Changed to false according to SpO2 example
  const int kAveragingSamples = 5; // Changed to 5 according to SpO2 example
  ...
  // Variables to hold measurement results
  float heartRate = 0;
  bool validHeartRate = false;
  float spO2 = 0;
  bool validSpO2 = false;

  // Start measurement in a way that allows interruption
  uint32_t startTime = millis();
  uint32_t measurementDuration = 15000; // 15 seconds max measurement time

  // Main measurement loop with button check
  while (millis() - startTime < measurementDuration) {
    ...
    if (digitalRead(MENU_BTN_PIN) == ACTIVE_LOW || 
      ...
      display.fillRect(0, 30, 200, 100, GxEPD_BLACK);
      display.setCursor(0, 50);
      display.println("Measurement cancelled");
      display.display(false); // full refresh
      delay(1000);
      sensor.shutdown(); // 关闭MAX30105传感器
      showMenu(menuIndex, false);
      return;
    }
    
    // According to the official example, use readSample(1000) method
        ...
        if(crossed && current_diff < kEdgeThreshold) {
          if(last_heartbeat != 0 && crossed_time - last_heartbeat > 300) {
            ...
            if(bpm > 50 && bpm < 250) {
              // Average?
              if(kEnableAveraging) {
                int average_bpm = (int)round(averager_bpm.process((float)bpm));
    ...
    // Update display every second to show current readings
    static uint32_t lastUpdate = millis();
    if (millis() - lastUpdate > 1000) {
      display.fillRect(0, 60, 200, 40, GxEPD_BLACK);
      display.setCursor(0, 75);
      if (validHeartRate) {
        display.printf("HR: %.1f bpm", heartRate);
      } else {
        display.printf("HR: -- bpm");
      }
      display.setCursor(0, 95);
      if (validSpO2) {
        display.printf("SpO2: %.1f%%", spO2);
      } else {
        display.printf("SpO2: -- %%");
      }
      display.display(true); // partial refresh
      lastUpdate = millis();
      Serial.printf("DEBUG: Current readings - HR: %.1f bpm, SpO2: %.1f%%\n", heartRate, spO2);
    }
    
    delay(2); // Small delay but keep updating frequently
    yield(); // Allow other processes to run
  }

  Serial.println("DEBUG: Measurement loop completed");
  ...
  display.printf("Measurement completed");
  display.display(false); // full refresh
  Serial.printf("DEBUG: Final results displayed - HR: %.1f bpm, SpO2: %.1f%%\n", heartRate, spO2);
  
  // Wait for user to see results or press button
  unsigned long finalWaitStart = millis();
  Serial.println("DEBUG: Waiting for user to view results");
  while(millis() - finalWaitStart < 5000) { // Wait 5 seconds or until button press
    // Check if any button is pressed with debouncing
    if(digitalRead(MENU_BTN_PIN) == ACTIVE_LOW || 
       digitalRead(BACK_BTN_PIN) == ACTIVE_LOW || 
       digitalRead(UP_BTN_PIN) == ACTIVE_LOW || 
...
weatherData Watchy::_getWeatherData(String cityID, String lat, String lon, String units, String lang,
  ...
  if (weatherIntervalCounter >=
    ...
    if (connectWiFi()) {
      ...
      #else
      uint8_t temperature = 25; // Default temperature when BMA is disabled
      #endif
